	...
	chrome.system.cpu
			.getInfo(function(info) {
				...
				for (var i = 0; i < info.processors.length; i++) {
					total += info.processors[i].usage.total
							- cpu_history.last_total[i];
					cpu_history.last_total[i] = info.processors[i].usage.total;
					user += info.processors[i].usage.user
							- cpu_history.last_user[i];
					cpu_history.last_user[i] = info.processors[i].usage.user;
					kernel += info.processors[i].usage.kernel
							- cpu_history.last_kernel[i];
...
function updateMemHistory() {
	getMemUsage(function(usage) {
		...
		mem_history.used.push(Math
				.round((usage.capacity - usage.availableCapacity)
						/ usage.capacity * 100));
		showMem();
	});
}
...
function showCpu() {
	...
	cpu_chart_line.Line(history, {
		scaleFontSize : 4,
		pointDot : false,
		animation : false
	});
	cpu_chart_pie.Pie(now, {
...
function showMem() {
	...
	mem_chart_line.Line(history, {
		scaleFontSize : 4,
		pointDot : false,
		animation : false
	});
	mem_chart_pie.Pie(now, {
		segmentShowStroke : false,
		animation : false
	});
}
